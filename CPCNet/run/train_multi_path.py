...
def train_multi_path():
    """
    Train from scratch or from checkpoints.
    """

    global model, loss_fn, optimizer, train_dataset, val_dataset, test_dataset, avg_loss_a, avg_loss_b, \
    ...
    train_dataset = get_RAVEN_dataset(args.dataset_path, "train", args.train_configs, args.train_configs_proportion)
    ...
    test_dataset = get_RAVEN_dataset(args.dataset_path, "test", args.train_configs).map(map_func = read_RAVEN_file, num_parallel_calls = args.num_workers).batch(batch_size = args.batch_size)

    avg_loss_a = tf.keras.metrics.Mean()
    avg_loss_b = tf.keras.metrics.Mean()
    ...
    if exists(ckpt_path) and exists(log_path):
        ...
        manager = tf.train.CheckpointManager(ckpt, directory = ckpt_path, max_to_keep = 100)
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        train_epoch()
        loss = {'train_a': avg_loss_a.result().numpy(), 'train_b': avg_loss_b.result().numpy()}
        ...
        validate_epoch()
        loss['val_a'] = avg_loss_a.result().numpy()
        loss['val_b'] = avg_loss_b.result().numpy()
        ...
        print(f"Train/Val: \nLoss_a: {loss['train_a']:.7f} / {loss['val_a']:.7f} \nLoss_b: {loss['train_b']:.7f} / {loss['val_b']:.7f} \nAcc: {acc['train']:.4f} / {acc['val']:.4f}")
...
def train_epoch():
    ...
    global train_dataset, avg_loss_a, avg_loss_b, avg_acc, avg_acc_dc, avg_acc_d9, avg_acc_d4, avg_acc_cc, avg_acc_lr, avg_acc_ud, avg_acc_cs

    avg_loss_a.reset_state()
    avg_loss_b.reset_state()
    avg_acc.reset_state()
    ...
    for images, targets, configs in pbar:
        ...
        loss_a, loss_b = tf_func(images, targets, configs)

        pbar.set_postfix(loss_a = f"{loss_a.numpy():.7f}", loss_b = f"{loss_b.numpy():.7f}")
...
def tf_func(images, targets, configs):
    ...
    with tf.GradientTape() as tape:
        logits_a, logits_b = model(images, training = True)
    ...
    avg_loss_a.update_state(loss_a)
    avg_loss_b.update_state(loss_b)
...
def validate_epoch():
    ...
    global val_dataset, avg_loss_a, avg_loss_b, avg_acc, avg_acc_dc, avg_acc_d9, avg_acc_d4, avg_acc_cc, avg_acc_lr, avg_acc_ud, avg_acc_cs
